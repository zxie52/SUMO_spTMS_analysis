function [pow, phase, comp, dstimes, freqs] = dothewave(timeseries, srate, freqrange, nfreqs, cyclerange, dsfac, baseline, timevec)
% dsfac = 5;
...
% nfreqs = # of frequencies you want to estimate in the range -- logrithmatically spaced
...
num_frex = nfreqs;
freqs = linspace(min_freq,max_freq,num_frex);
...
parfor ch = 1:size(timeseries,1)
    ...
    if length(size(timeseries)) == 3 && isempty(baseline)
        pow(ch,:,:,:) = abs(tf(:,1:dsfac:end,:)).^2; % no need for the abs()
    ...
    elseif length(size(timeseries)) == 2 && isempty(baseline)
        ...
        if length(size(timeseries))==3
            ...
            phase(ch,:,:,:) = angle(tf(:,1:dsfac:end,:));
            comp(ch,:,:,:) = tf(:,1:dsfac:end,:);
        elseif length(size(timeseries)) == 2
