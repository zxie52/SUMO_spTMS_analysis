...
if strcmp(condition, 'testorient')
    fpath13 = 'E:\SUMO_further_data_pack_zx\N2pc_IEM\new_results\IEM_results_voltage_probe2_testorient';
end

if strcmp(condition, 'targetorient')
    fpath13 = 'E:\SUMO_further_data_pack_zx\N2pc_IEM\new_results\IEM_results_voltage_probe2_targetorient';
end
...
for l = 1:length(subject)
    ...
    for t = 7 % This time we only preprocess the Probe1 data
        ...
        if strcmp(condition, 'testorient')
            ...
            for i = 1 : height(p)
                p.testorient2(i) = p.targetorient2(i) + p.accuracy2(i);
            end
            ...
            for i = 1 : height(p)
                if p.testorient2(i) > 180
                    p.testorient2(i) = p.testorient2(i) - 180;
                elseif p.testorient2(i) < 0
                        p.testorient2(i) = 180 + p.testorient2(i);
                end
            end
            % move the testorient column right after the targetorient column
            p = movevars(p, 'testorient2', 'After', 'targetorient2');
        end 
...
save(strcat('IEM_Exp1_contraposterior_allsubjects_epoch1_probe2_iem.mat'), 'dstimes', 'stimlabels', 'chanrespsL_allsub', 'chanresp_permsL_allsub', 'chanrespsR_allsub', 'chanresp_permsR_allsub'); 
...
ylim([-.2 .6]);
if strcmp(condition, 'testorient')
    title('IEM Left Stimulus Reconstruction from Contraposterior Electrodes(probe2 testorient)');
else
    title('IEM Left Stimulus Reconstruction from Contraposterior Electrodes(probe2 targetorient)');
end
xlabel('Centered Orientation Channel (°)')
...
if strcmp(condition, 'testorient')
    title('IEM Right Stimulus Reconstruction from Contraposterior Electrodes(probe2 testorient)');
else
    title('IEM Right Stimulus Reconstruction from Contraposterior Electrodes(probe2 targetorient)');
end
xlabel('Centered Orientation Channel (°)')
ylabel('Channel Response (µV^2)')
legend({'error', '-100-0ms','error','20-200ms','error','200-400ms'})
...
for k =  1 : 2
    ...
    if k == 1
        ...
        if strcmp(condition, 'testorient')
            title('Reconstruction from Voltage in ContraPosterior Channels Left probe(probe2)(testorient)', 'Fontsize', titleFontSize)
        elseif strcmp(condition, 'targetorient')
            title('Reconstruction from Voltage in ContraPosterior Channels Left probe(probe2)(targetorient)', 'Fontsize', titleFontSize)
        end            
    ...
    elseif k == 2 
        ylabel(h, 'Channel Response (µV^2)','Fontsize', axisFontSize)
        if strcmp(condition, 'testorient')
            title('Reconstruction from Voltage in ContraPosterior Channels Right probe(probe2)(testorient)', 'Fontsize', titleFontSize)
        elseif strcmp(condition, 'targetorient')
            title('Reconstruction from Voltage in ContraPosterior Channels Right probe(probe2)(targetorient)', 'Fontsize', titleFontSize)
        end   
        xlabel('Time from probe onset(msec)','Fontsize', axisFontSize)
        ylabel('Centered Orientation Channel (°)','Fontsize', axisFontSize)
