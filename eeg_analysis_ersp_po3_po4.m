...
while ~ismember(condition, {'lp'})
    warning("For frontal midline, please check the MATLAB code: eeg_analysis_ersp_frontal_midline.m")
    warning("The input condition is not available, please re-type the condition: lp");
...
for t = 2 %1:length(type)
    ...
    for i = 1:2
        ...
        saveas(gcf, strcat("allsubjects_ersp_left_", type{t},"_", ROI{i},"_heatmap_fdr_corrected.svg"));
    end
    ...
    for i = 1:2
        ...
        saveas(gcf, strcat("allsubjects_ersp_right_", type{t},"_", ROI{i},"_heatmap_fdr_corrected.svg"));
    end
    
    % contra - ipsi = PO3 - PO4
    contra_ipsi = pow_right_group(1,:,dstimes2,:) - pow_right_group_mean(2,:,dstimes2,:);
    imagesc(dstimes3, freqs, squeeze(mean(contra_ipsi,4)));
    ersp_fig_setup();
