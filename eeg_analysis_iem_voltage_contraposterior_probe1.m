...
if strcmp(condition, 'testorient')
    fpath13 = 'E:\SUMO_further_data_pack_zx\N2pc_IEM\new_results\IEM_results_voltage_probe1_testorient';
end

if strcmp(condition, 'targetorient')
    fpath13 = 'E:\SUMO_further_data_pack_zx\N2pc_IEM\new_results\IEM_results_voltage_probe1_targetorient';
end
...
save(strcat('IEM_Exp1_contraposterior_allsubjects_epoch1_probe1_iem.mat'), 'dstimes', 'stimlabels', 'chanrespsL_allsub', 'chanresp_permsL_allsub', 'chanrespsR_allsub', 'chanresp_permsR_allsub'); 
...
for k =  1 : 2
    ...
    % Plot the heatmap in the full size
    set(gcf, 'Position', get(0, 'Screensize'));
    % determine if it is left or right channel response
    if k == 1
        ylabel(h, 'Channel Response (µV^2)','Fontsize', axisFontSize)
        if strcmp(condition, 'testorient')
            title('Reconstruction from Voltage in ContraPosterior Channels Left probe(Probe1)(testorient)', 'Fontsize', titleFontSize)
        elseif strcmp(condition, 'targetorient')
            title('Reconstruction from Voltage in ContraPosterior Channels Left probe(Probe1)(targetorient)', 'Fontsize', titleFontSize)
        end            
    ...
    elseif k == 2 
        ylabel(h, 'Channel Response (µV^2)','Fontsize', axisFontSize)
        if strcmp(condition, 'testorient')
            title('Reconstruction from Voltage in ContraPosterior Channels Right probe(Probe1)(testorient)', 'Fontsize', titleFontSize)
        elseif strcmp(condition, 'targetorient')
            title('Reconstruction from Voltage in ContraPosterior Channels Right probe(Probe1)(targetorient)', 'Fontsize', titleFontSize)
        end   
        xlabel('Time from probe onset(msec)','Fontsize', axisFontSize)
        ylabel('Centered Orientation Channel (°)','Fontsize', axisFontSize)
